...
jqSimpleConnect._positionConnection = function(connection) {
    ...
    posA.left = posA.left + parseInt(connection.elementA.width() / 2, 10);
    posA.top = posA.top + parseInt(connection.elementA.height() / 2, 10);

    var posB = connection.elementB.offset();
    posB.left = posB.left + parseInt(connection.elementB.width() / 2, 10) + 2;
    posB.top = posB.top + parseInt(connection.elementB.height() / 2, 10) + 4;
...
jqSimpleConnect.connect = function(elementA, elementB, options) {
    ...
    var div = "<div id=\"divUniqueIdentifier\" class=\"jqSimpleConnect " + connection.id + "\" " +
        ...
        "position:absolute; z-index: 10000;\"></div>";
